...
all %>%
  drop_na(infant_asq_total_score, MGB053) %>%
  ggplot(aes(
    x = infant_asq_total_score,
    y = MGB053
  )) +
  geom_point(alpha = 0.7, size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
    title = "Butyrate Synthesis II",
    x = "Total ASQ Score",
    y = "MGB053 Abundance"
  ) +
  scale_color_brewer(palette = "Set2") +
...
# Remove NAs
df_plot <- all %>%
  drop_na(infant_asq_total_score, MGB053, infant_ffq_feeding_mode_complex)

# Fit linear model
lm_fit <- lm(MGB053 ~ infant_asq_total_score, data = df_plot)
...
ggplot(
  ...
  aes(
    x = infant_asq_total_score,
    y = MGB053,
    color = infant_ffq_feeding_mode_complex
  )
) +
  geom_point(alpha = 0.7, size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
