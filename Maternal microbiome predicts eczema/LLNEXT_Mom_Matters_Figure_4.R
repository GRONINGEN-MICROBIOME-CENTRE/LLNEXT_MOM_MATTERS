...
# This script gives the code for Main figure 4 and related supplementary tables 
# Last update: 8th July, 2026, adding annotations to mention links to supplementary tables and source data
...
# Source data 4a 
write.table(merged_alpha_sig, "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/source_data_fig_4_a_merged_alpha_sig_mother.txt", sep="\t", row.names = F)
...
# Source data 4b
write.table(significant2 , "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/source_data_fig_4_b_merged_adonis.txt", sep="\t", row.names = F)
...
source_data_4_d<-eczema %>%
  select(Timepoint_categorical, shannon, infant_health_eczema_diagnosis_strict)

# Source data 4d
write.table(source_data_4_d , "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/source_data_fig_4_d_eczema.txt", sep="\t", row.names = F)
...
source_data_4_c<-delivery_place_simple %>%
  select(Timepoint_categorical, shannon, birth_deliverybirthcard_place_delivery_simple)

# Source data 4c
write.table(source_data_4_c , "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/source_data_fig_4_c_delivery_place.txt", sep="\t", row.names = F)
...
# Load scripts 
source("~/Desktop/LLNEXT/Analysis/scripts/new_scripts_2026/Basic_functions_XGBoost.R")
...
##############################
# Export source data Figure 4E-F
##############################

outdir <- "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/"

##############################
# Figure 4E: ROC source data
##############################

# Helper function to extract ROC curve coordinates
extract_roc_source <- function(roc_obj, model_name) {
  tibble(
    model = model_name,
...
source_data_fig_4e_roc <- bind_rows(
  extract_roc_source(roc1, "SGBs"),
  extract_roc_source(roc2, "Pathways"),
  extract_roc_source(roc3, "Alpha diversity")
)

write.table(
  source_data_fig_4e_roc,
  file = file.path(outdir, "source_data_fig_4e_ROC_curves.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)

# Also export sample-level predicted probabilities
source_data_fig_4e_predictions <- tibble(
  NG_ID = rownames(final_data),
  NEXT_ID = final_data$NEXT_ID,
  eczema_diagnosis_strict = final_data$infant_health_eczema_diagnosis_strict,
  eczema_binary = as.numeric(final_data$infant_health_eczema_diagnosis_strict == "yes"),
  predicted_probability_SGBs = Pred_probabilities,
  predicted_probability_pathways = Pred_probabilities_path,
  predicted_probability_alpha_diversity = Pred_probabilities_alpha,
  shannon = final_data$shannon
)

write.table(
  source_data_fig_4e_predictions,
  file = file.path(outdir, "source_data_fig_4e_predicted_probabilities.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)

# Optional: export model scores/AUC table
source_data_fig_4e_scores <- tibble(
  model = c("SGBs", "Pathways", "Alpha diversity"),
  auc = c(as.numeric(auc1), as.numeric(auc2), as.numeric(auc3))
)

write.table(
  source_data_fig_4e_scores,
  file = file.path(outdir, "source_data_fig_4e_AUC_values.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
...
##############################
# Figure 4F: SHAP/source data for top SGB features
##############################
...
source_data_fig_4f_shap_importance <- tibble(
...
write.table(
  source_data_fig_4f_shap_importance,
  file = file.path(outdir, "source_data_fig_4f_SHAP_feature_importance.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)

# Make sure SHAP matrix and SGB matrix use the same sample IDs
rownames(shap_matrix) <- rownames(SGB_filtered_prev_mat)

source_data_fig_4f_shap_beeswarm <- shap_matrix[, top10_features, drop = FALSE] %>%
  as.data.frame() %>%
  rownames_to_column("sample_id") %>%
  pivot_longer(
    cols = all_of(top10_features),
    names_to = "feature",
    values_to = "SHAP_value"
  ) %>%
  left_join(
    SGB_filtered_prev_mat[, top10_features, drop = FALSE] %>%
      as.data.frame() %>%
      rownames_to_column("sample_id") %>%
      pivot_longer(
        cols = all_of(top10_features),
...
write.table(
  source_data_fig_4f_shap_beeswarm,
  file = file.path(outdir, "source_data_fig_4f_SHAP_beeswarm_top10.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
