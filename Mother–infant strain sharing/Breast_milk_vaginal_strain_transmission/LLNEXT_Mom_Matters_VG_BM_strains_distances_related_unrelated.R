################## Vaginal & Breastmilk distances between related versus unrelated mother infant pairs #######################
# Author: T. Sinha 
# Date: 14th of August, 2025
# Scripts to visualize and test distances between Vaginal & breast milk distances between related versus unrelated mother infant pairs
# Also contains scripts to plot Extended Data 7b 
...
final_results_milk_fil <- final_results_milk_fil %>%
  mutate(
...
combined_vg_bm_dist<-plot_grid(p_milk, p_vg) 

ggsave(
  filename = "/Users/trishlasinha/Desktop/LLNEXT/Analysis/results/figures/supplementary/ED_7b_combined_vg_bm_dist.pdf",
  plot = combined_vg_bm_dist,
  device = cairo_pdf,
  width = 500,
  height = 200,
  units = "mm"
...
source_data_milk <- distances_BM %>%
  ...
  dplyr::mutate(
    Group = dplyr::recode(
      Group,
      mean_related = "Related",
      mean_unrelated = "Unrelated"
    ),
    Significant = p_value < 0.05
  ) %>%
  dplyr::arrange(species_SGB, Group)

write.csv(
  source_data_milk,
  "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/Source_Data_7b_Breastmilk_Related_Unrelated_Distances.csv",
  row.names = FALSE
)


source_data_vg <- distances_VG %>%
  dplyr::select(
    Species,
    SGB,
    species_SGB,
...
write.csv(
  source_data_vg,
  "/Users/trishlasinha/Desktop/LLNEXT/Analysis/submission_Nature_post_acceptance_2026/Source_data/Source_Data_7b_Vaginal_Related_Unrelated_Distances.csv",
  row.names = FALSE
)
